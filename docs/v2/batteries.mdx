...
| [permissions](#permissions) | Array[String] | Read/Write\* | Permissions to allow charging, discharging, both, or neither.                       |
| battery_count               | Number        | Read-only    | Number of connected Plug-In Batteries.                                              |
...
Permissions is read-only in `to_full` mode.
