...
| to_full                     | Boolean       | Read/Write   | Whether the Plug-In Battery should be charged to full. This will replace the `to_full` mode. When `to_full` mode is enabled, this field will be set to `true` and all the batteries will charge to 100%, when all batteries are fully charged, this field will be set to `false` and the mode will switch to the previous mode. You can turn off `to_full` at any time. | <Badge colorName='hw-purple' icon="mdi:test-tube" detail='API 2.3.0'>Beta</Badge>                                                                                  |
