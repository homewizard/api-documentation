...
:::danger
This is not semver, but a custom versioning scheme.
:::

-   The version is specified in the format `X.Y.Z`, where `X` is the major version, `Y` is the minor version and `Z` is the patch version.
-   The major will always be `2` until further notice.
-   The minor will be incremented for each new feature or change which is not backwards compatible.
-   The patch will be incremented for each bug fix or backwards compatible change.

--- or

:::tip
This is semver, but means the 'V2' API will become 'V3 etc' for each breaking change.
:::

-   The version is specified in the format `X.Y.Z`, where `X` is the major version, `Y` is the minor version and `Z` is the patch version.
-   The major will be incremented for each breaking or incompatible change.
-   The minor will be incremented for each new feature or change, which is backwards compatible. This includes new endpoints, new parameters, new actions, etc.
-   The patch will be incremented for each bug fix or backwards compatible change. This includes changes to existing endpoints, parameters, actions, etc.
