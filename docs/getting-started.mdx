...
The HomeWizard Energy API V2 is an updated version of the API that is currently in beta. This API is based on HTTPS and uses authorization to access the data, so you don't need to enable the local API. You can find the documentation for the V2 API [here](/docs/v2).
...
`curl https://<IP ADDRESS>/api/user --insecure -X POST -d '{"name": "cool-new-app"}'`
...
1. Send the following request to the device to get basic information. Replace `<TOKEN>` with the token you received in the previous step: `curl https://<IP ADDRESS>/api/measurement --insecure -H "Authorization: Bearer <TOKEN>"`
