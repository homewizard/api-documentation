...
# P1 Telegram `/api/v2/telegram`

<PreliminaryWarning />

The `/api/v2/telegram` endpoint returns the most recent, valid telegram that was given by the P1 meter.
...
-   If you need parsed data, use the [measurement](/docs/V2/endpoints/measurement) endpoint.
...
```http title="Request"
curl https://<IP ADDRESS>/api/v2/telegram --insecure \
