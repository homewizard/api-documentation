...
A user is created by sending a POST /api/v2/user request when they press the button on the device. This action confirms that the user has possession of the device and that the application is authenticated to generate a token. Once these steps are completed, a token is returned which is used to authorize for future requests.
...
| token | String | A token that can be used to authorize API requests.<br />- The token is 32 characters long.<br />- Token consists of uppercase letters (`A-Z`) and numbers (`0-9`).<br />&nbsp;&nbsp;- (Regex: `^[A-Z0-9]{32}$`) |
...
The hostname can be obtained via [mDNS](/docs/discovery#api-v2) or by combining the product type and serial number of the device. Due to legacy reasons, the product type set in the certificate is different from the actual product type.
...
print(result) # prints {"error":"user:unauthorized"}, which is good.
```
