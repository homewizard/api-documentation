...
The WebSocket is accessible at `wss://<IP ADDRESS>/api/v2/ws`. The connection is secured with the same [certificate as the HTTPS connection](/docs/v2/authorization#ssl-certificate-validation-https-https). Once connected, you must send the authorization token within 40 seconds, or the connection will close.
