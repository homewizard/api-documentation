...
To connect to the WebSocket, you must obtain an authentication token. Refer to this [page](/docs/v2/authentication#create-user-and-get-token) for instructions.

The WebSocket is accessible at `wss://<IP ADDRESS>/api/v2/ws`. The connection is secured with the same [certificate as the HTTPS connection](/docs/v2/authentication.mdx#ssl-certificate-validation-https-https). Once connected, you must send the authentication token within 40 seconds, or the connection will close.
...
{"type": "authentication", "data": "<TOKEN>"}
...
-   **`device`** - Subscribe to [device information](http://localhost:3000/docs/V1/device_information) updates
-   **`user`** - Subscribe to [user list](http://localhost:3000/docs/V1/authentication) updates
-   **`measurement`** - Subscribe to [measurement](http://localhost:3000/docs/V1/measurement) updates
-   **`system`** - Subscribe to [system](http://localhost:3000/docs/V1/system) updates. Only `cloud_enabled` and `status_led_brightness_pct` are updated in real time.
