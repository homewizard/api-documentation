...
The `/api/v2/telegram` endpoint returns the most recent, valid telegram that was given by the P1 meter, therefore this endpoint is **only available for the P1 Meter (HWE-P1)**.
...
```
<Request>
GET https://{IP address}/api/v2/telegram
