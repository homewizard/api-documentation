...
| monthly_power_peak_timestamp | [Timestamp](#timestamps)         | Timestamp when peak demand was registered                                                                                  |
| external                     | Array with [External](#external) | A list of externally connected utility meters, see [External data point](#external-data-point) for more information        |

### External devices {#external}

Some smart meters have one or more external device connected to it. This can be, for example, a gas and a water meter. The `external` data point list these devices and their measurements.
