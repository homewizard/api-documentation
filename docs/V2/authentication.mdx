...
-   Add details about token validity, revocation, etc.
-   Specify required parameters for users
-   Explain how long the initial request remains valid and the time window for pressing the button
-   Inform users that tokens for battery pairing can also be generated via the cloud
-   Add GET and DELETE method descriptions
...
The name provided in [Parameters](#parameters).
...
:::danger
Keep the token secure, as it grants full access to the device, including the ability to generate new tokens.
:::
