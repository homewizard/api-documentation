...
-   [`_hwenergy._tcp`](#_hwenergy.tcp) - to discover devices that are using API v1 over HTTP
-   [`_homewizard._tcp`](#_homewizard_tcp) - to discover devices that are using API 2.0.0 and later over HTTPS
...
| product_name | User-friendly name. This name may change at any time.                                                                                                                      |
...
This service is used to discover devices that are using **API 2.0.0 and later**, which uses HTTPS (port 443). Devices with this API version are available on the `_homewizard._tcp` service.
